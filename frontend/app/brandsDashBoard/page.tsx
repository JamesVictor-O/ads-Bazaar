...
const campaignData = [
  {
    id: 1,
    name: "Celo Mobile Banking Promo",
    status: "active",
    spent: 2430,
    budget: 5000,
    impressions: 154200,
  ...
  1: [
    {
      ...
      followers: 12000,
      message:
        "Excited to promote Celo’s mobile banking! I have a strong DeFi audience.",
    },
    {
      address: "0x456...def",
      username: "web3_queen",
      followers: 8000,
      message: "Perfect fit for my followers interested in fintech.",
    },
    {
      address: "0x789...ghi",
      username: "blockchain_bro",
      followers: 5000,
      message: "Can create engaging content for this campaign.",
    },
  ],
  4: [
    {
      ...
      username: "gamer_nft",
      followers: 15000,
      message: "My gaming audience will love this tournament!",
    },
  ],
};
  ...
  1: [
    {
      address: "0x111...xyz",
      username: "defi_star",
      postLink: "https://farcaster.com/defi_star/post/123",
      paid: false,
    },
    {
      address: "0x222...uvw",
      username: "celo_fan",
      postLink: "https://x.com/celo_fan/status/456",
      paid: true,
    },
  ],
  4: [
    {
      address: "0x333...rst",
      username: "game_king",
      postLink: "https://farcaster.com/game_king/post/789",
      paid: false,
    },
  ],
};
...
const BrandDashboard = () => {
  ...
  const totalInfluencers = mockCampaigns.reduce(
    (sum, item) => sum + item.influencers,
    0
  );
  ...
  const handleAssignInfluencer = (campaignId, applicantAddress) => {
    setMockCampaigns((prev) =>
      prev.map((campaign) =>
        ...
        campaign.influencers < campaign.maxInfluencers
    ...
    mockSubmissions[campaignId] = [
      ...(mockSubmissions[campaignId] || []),
      {
        ...
        username: mockApplications[campaignId].find(
          (a) => a.address === applicantAddress
        ).username,
        postLink: "",
        paid: false,
      },
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-auto max-h-[80vh] overflow-y-auto">
            <h2 className="text-lg text-gray-700 font-medium mb-4">
              Applications for {selectedCampaign.name}
            </h2>
            <ul className="divide-y divide-gray-200">
              {(mockApplications[selectedCampaign.id] || []).map(
                (applicant, index) => (
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            Followers: {applicant.followers.toLocaleString()}
                          </p>
                        </div>
                        {selectedCampaign.influencers <
                          selectedCampaign.maxInfluencers && (
                          <button
                            onClick={() =>
                            ...
                            disabled={mockSubmissions[
                              selectedCampaign.id
                            ]?.some((sub) => sub.address === applicant.address)}
                          >
                            Assign
                          </button>
                        )}
      ...
      {showSubmissionsModal && selectedCampaign && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-auto max-h-[80vh] overflow-y-auto">
            <h2 className="text-lg text-gray-700 font-medium mb-4">
              Submissions for {selectedCampaign.name}
            </h2>
            <ul className="divide-y divide-gray-200">
              {(mockSubmissions[selectedCampaign.id] || []).map(
                (submission, index) => (
                  <li key={index} className="py-4">
                    <div className="flex flex-col gap-2">
                      <div className="flex justify-between items-start">
