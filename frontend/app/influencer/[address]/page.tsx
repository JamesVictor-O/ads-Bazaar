...
import { getUserStatusLabel } from "@/utils/format";
...
function EnhancedInfluencerProfileComponent({
  ...
  const { applications, isLoadingApplications } = useBriefApplications(
  ...
  const selectedCount = applications
    ? applications.filter((app) => app.isSelected).length
    : 0;
  ...
  const canAssign =
    spotsRemaining > 0 && currentApplication && !currentApplication.isSelected;
  ...
  const { data: influencerProfile, refetch: refetchInfluencerProfile } =
    useGetInfluencerProfile(profileAddress as `0x${string}`);
  ...
  const handleAssignCampaign = async () => {
    ...
    await guardedAction(async () => {
      try {
        ...
        console.log(
          "DIVVI: About to assign campaign with referral tag:",
          referralTag
        );

        await selectInfluencer(
          briefId as `0x${string}`,
          currentAppIndex,
          referralTag
        );
  ...
  const handleNextApplication = () => {
    ...
    router.push(
      `/influencer/${nextApplication.influencer}?from=applications&briefId=${briefId}&appIndex=${nextIndex}`
    );
  };
  ...
  const socialMediaData = influencerProfile
    ? JSON.parse(influencerProfile as string)
    : {};
  ...
  const getDisplayName = () => {
    ...
    return profileAddress
      ? `Creator ${(profileAddress as string).slice(0, 6)}...${(
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-lg mx-auto min-h-screen overflow-y-auto">
        ...
        <div className="relative mt-10">
          ...
          <div className="relative px-2 pt-16 sm:px-4 sm:pt-24 md:pt-28 pb-4 sm:pb-6">
            {/* Profile Header */}
            <motion.div
            ...
            >
              ...
              <div className="relative inline-block mb-4 sm:mb-6">
                <div className="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto">
                  ...
                  {isVerified && (
                    <motion.div
                      className="absolute -bottom-1 -right-1 w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-full border-3 border-slate-900 flex items-center justify-center shadow-lg"
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                <span
                  className={`inline-flex items-center px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-semibold border ${
                    isVerified
                ...
                >
                  {isVerified ? (
                    <>
                      <Verified className="w-4 h-4 mr-1 sm:mr-2" />
                      Verified Creator
                    </>
                  ) : (
                    <>
                ...
                <span className="inline-flex items-center px-2.5 py-1 sm:px-3 sm:py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400 border border-blue-500/40 mt-1 sm:mt-0">
              ...
              {/* Address */}
              <div className="flex flex-row  items-center justify-center  sm:gap-2 mb-3 sm:mb-4">
                <span className="bg-slate-800/60 px-2.5 py-1 rounded-lg text-xs sm:text-sm text-slate-300 border border-slate-700/50">
                  <UserDisplay
                ...
                <button
                  ...
                  className="text-slate-500 hover:text-emerald-400 transition-colors p-1 rounded-lg hover:bg-slate-800/50"
                >
                  {copiedText === "Address" ? (
              ...
              {/* Bio */}
              <p className="text-slate-300 text-sm sm:text-base leading-relaxed max-w-xs sm:max-w-md mx-auto px-1">
                {bio}
              </p>
            </motion.div>
          </div>
        ...
        <div className="px-2 sm:px-4 mb-4 sm:mb-6">
          <div className="flex flex-row bg-slate-800/50 rounded-2xl p-1 border border-slate-700/50 gap-2">
            {[
              ...
              <motion.button
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-2 py-2 sm:px-4 sm:py-3 rounded-xl font-medium transition-all text-xs sm:text-base whitespace-nowrap ${
                  activeTab === tab.key
              ...
              >
                ...
                <div className="flex flex-row gap-2 sm:gap-4">
                  {stats.map((stat, index) => (
                    <motion.div
                      key={stat.label}
                      className="flex-1 bg-slate-800/40 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-2 sm:p-4 text-center min-w-0"
                    ...
                    >
                      ...
                      <p className="text-base sm:text-xl md:text-2xl font-bold text-white mb-1 sm:mb-2">
                        {stat.value}
                      </p>
                ...
                <div className="bg-gradient-to-r from-emerald-500/10 to-blue-500/10 border border-emerald-500/20 rounded-2xl p-4 sm:p-6 mb-6 flex flex-col items-center justify-center">
          ...
          <div className="space-y-4">
            ...
            {isOwner && !fromApplications && (
              <Link href="/influencersDashboard">
                <motion.button
                  className="w-full flex items-center justify-center gap-3 md:px-6 md:py-4 py-2 mb-3 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-2xl font-semibold transition-all shadow-lg shadow-emerald-500/20"
                  whileTap={{ scale: 0.95 }}
                >
            ...
            {!fromApplications && (
              <div className="flex flex-col gap-3 sm:flex-row">
                <motion.button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-4 py-3 sm:py-2 bg-slate-800/60 hover:bg-slate-800 text-base text-slate-300 rounded-xl border border-slate-600/50 transition-all font-medium"
                  whileTap={{ scale: 0.95 }}
                >
                ...
                <Link href="/marketplace" className="w-full sm:w-auto">
                  <motion.button
                    className="w-full flex items-center justify-center gap-3 px-4 py-3 sm:py-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl transition-all font-medium shadow-lg shadow-blue-500/20"
                    whileTap={{ scale: 0.95 }}
                  >
                    {isOwner ? (
                      <>
